...
def executar_calculos(perfil, fy, Lx, Ly, Lz, Cb, c, debug=False):
    ...
    alma_y_esc = calcular_alma_y_esc(perfil, fy, mesa_y_esc.get("xg2", 0), mesa_y_esc.get("Aef2", 0))
    ...
    alma_y_flt = calcular_alma_y_flt(perfil, fy_y_flt, mesa_y_flt.get("xg2", 0), mesa_y_flt.get("Aef2", 0))
    ...
    # --- Definições Geométricas Básicas (para Cortante e Crippling) ---
    try:
        bw = perfil["bw"]
        t = perfil["t"]
        h = bw - 4 * t # Altura livre da alma
    except KeyError as e:
        print(f"Erro: {e} não encontrado no dicionário 'perfil'.")
        h = 0 # Evita falhas subsequentes
        t = 0
    except Exception as e:
        print(f"Erro ao definir h e t: {e}")
        h = 0
        t = 0

    # --- Web Crippling ---
    Fwrd_ext, Fwrd_int = 0.0, 0.0 # Inicializa
    try:
        # ri DEVE VIR DO PERFIL DO CATÁLOGO, não é 2*t
        ri = perfil.get("ri", 2 * t) # Usa 'ri' do perfil, ou 2*t se não existir
        if debug and "ri" not in perfil:
            print(f"Debug (Perfil {perfil['perfil']}): 'ri' não encontrado. Usando fallback 2*t = {ri}")
        ...
        if all([cond1, cond2, cond3, cond4]):
            ...
            if debug: print(f"Debug (Perfil {perfil['perfil']}): Não atendeu aos requisitos de Web Crippling.")
            Fwrd_ext, Fwrd_int = 0.0, 0.0 # Condições não atendidas
    ...
    # --- Força Cortante ---
    Vrd, regime, kv = 0.0, "N/A", 0.0 # Inicializa
    try:
        if h > 0 and t > 0: # Evita divisão por zero
            # Kv
            if (Lx / h) < 3 and (Lx / h) < (260 / (h / t))**2:
                kv = 5
            else:
                kv = 5 + 5 / (Lx / h)**2
            ...
            if λ <= lim_inf:
             ...
             if debug: print(f"Debug (Perfil {perfil['perfil']}): h ou t é zero. Cálculo de cortante ignorado.")

    except Exception as e:
        print(f"Erro no cálculo de cortante: {e}")
        Vrd, regime, kv = 0.0, "Erro", 0.0
